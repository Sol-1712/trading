...
# Read
# ---------------------------------------------------------------------------
def get_latest_timestamp(path: str | Path, bound: Bound = "end") -> pd.Timestamp | None:
    """
    Return the most recent timestamp stored in a partitioned dataset,
    ...
    dataset_path : str | Path
        Root directory of the partitioned dataset.
    bound : Bound
        Whether to return the start or end timestamp of the latest partition.
    """
    ...
    try:
        ...
        if bound == "start":
            year_dir = year_dirs[0]  # Earliest year directory
        else:
            year_dir = year_dirs[-1] # Latest year directory
    ...
    if file_path.exists():
        ...
        logger.info("Latest partition found at %d/%d", month, year)
    else:
        logger.warning("No parquet files found in path: %s", path)
        return None
    
    if df.empty:
        logger.warning("get_latest_timestamp retrieved an empty DataFrame")
        return None
