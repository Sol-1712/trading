...
# Literal types let Pylance/mypy catch invalid price_type strings at analysis
# time rather than at runtime. Strictly better than a plain str argument.
PriceType = Literal["last", "mark", "index"]
KLINE_PRICE_TYPES: list[PriceType] = ["last", "mark", "index"]
...
_KLINE_SCHEMA: dict[str, dict[str, list[str]]] = {
    "last": {
...
_FUNDING_RAW_COLS: list[str] = ["funding_rate", "timestamp"]
...
class BybitFetcher:
    ...
    def _get_kline_api_method(self, price_type: PriceType):
        ...
        methods: dict[PriceType, Callable] = {
            "last": self._session.get_kline,
            "mark": self._session.get_mark_price_kline,
            "index": self._session.get_index_price_kline,
        }
    ...
    def fetch_funding_rate(
        ...
        def call_fn(s: int, e: int) -> pd.DataFrame:
            ...
            data = response["result"]["list"]
            if not data:
                return pd.DataFrame(columns=_FUNDING_RAW_COLS)
            # Rename Bybit's camelCase fields to snake_case immediately,
            # before any data leaves this closure.
            return pd.DataFrame(data, columns=["funding_rate", "timestamp"])
