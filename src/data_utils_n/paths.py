...
# Resolve project root: paths.py -> data_utils -> src -> project root.
PROJECT_ROOT = Path(__file__).resolve().parents[2]
DATA_ROOT = PROJECT_ROOT / "data" / "raw"

DEFAULT_EXCHANGE = 'bybit'

DataType = Literal["klines", "funding"]
PriceType = Literal["last", "mark", "index"]

_VALID_DATA_TYPES: frozenset[str] = frozenset({"klines", "funding"})
_VALID_PRICE_TYPES: frozenset[str] = frozenset({"last", "mark", "index"})
...
def make_data_path(
    ...
    data_type = data_type.lower()  # type: ignore[assignment]
    ...
    if data_type == "klines":
        ...
        path = base / "klines" / f"{interval}m" / price_type  # type: ignore[operator]
...
def _validate_data_type(data_type: str) -> None:
    if data_type not in _VALID_DATA_TYPES:
        raise ValueError(
            ...
            f"Valid options: {sorted(_VALID_DATA_TYPES)}"
        )


def _validate_klines_args(
    interval: int | None,
    price_type: str | None,
) -> None:
    if interval is None:
    ...
    if price_type not in _VALID_PRICE_TYPES:
        raise ValueError(
            f"Invalid price_type {price_type!r}. "
            f"Valid options: {sorted(_VALID_PRICE_TYPES)}"
        )